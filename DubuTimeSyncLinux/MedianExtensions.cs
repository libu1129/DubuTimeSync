...
{
    public static T Median<T>(this IEnumerable<T> src, Func<T, double> source)
    {
        var lst = src.OrderBy(p => source(p));
        var idx = (int)Math.Truncate((double)lst.Count() / 2d);
        return lst.ElementAt(idx);
    }
